			...
			<div id="container-painel-musico">
				<div id="container-editar-musico" style="height: none;">
					...
					<div id="caixa-de-entrada">
						<form action="salva-localidade.php" method="POST">
							<?php
							while($localidade = mysqli_fetch_assoc($localidades)){
								if($localidade['id'] == $_GET['i']){
								?>
								<input name="id_localidade" type="text" style="display: none" value="<?= $localidade['id']?>"></input><br>
								<label>Localidade: </label><input name="nome_localidade" type="text" value="<?= utf8_encode($localidade['localidade'])?>"></input>
