			...
			<div id="container-painel-musico">
				<div id="container-editar-musico" style="height: none;">
					...
					<div id="caixa-de-entrada">
						<form action="salva-genero.php" method="POST">
							<?php
							while($genero = mysqli_fetch_assoc($generos)){
								if($genero['id_genero'] == $_GET['i']){
								?>
								<input name="id_genero" type="text" style="display: none" value="<?= $genero['id_genero']?>"></input><br>
								<label>Gênero: </label><input name="nome_genero" type="text" value="<?= $genero['nome_genero']?>"></input>
